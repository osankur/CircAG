...
/** @brief
  *   Deterministic or nondeterministic LTS used as assumptions and properties.
  *
  * @param name
  * @param dfa
  * @param alphabet
...
trait LTS[S](
  ...
  def visualize(): Unit = {
    Visualization.visualize(
      dfa: Automaton[S, String, S],
      Alphabets.fromList(alphabet.toList)
    )
  }
...
object DLTS {
  ...
  /** @brief
    ...
    *   extendedAlphabet by copying it and adding self-loops at all states on
    *   symbols in extendedAlphabet \\ alphabet.
    *
    * @param dfa
    * @param alphabet
  ...
  def liftAndStripNonAccepting(
    ...
    val liftedDLTS = lift(
      DLTS(dlts.name, DFAs.complete(dlts.dfa, alph, tmpFastDFA), dlts.alphabet),
      extendedAlphabet,
      name
    )
  ...
  /** Make a DLTS that reads a given lasso; and make the first state of the
    * cycle accepting.
    *
    * @param lasso
    *   lasso
    * @param alphabet
    *   alphabet of the resulting automaton
    * @pre
    *   alphabet contains all symbols of lasso
    * @return
    */
  ...
  /** @brief
    *   Build deterministic LTS from possbly non-deterministic HOA Buchi
    ...
    */
  def fromHOAString(
      automatonString: String,
      fullAlphabet: Option[Alphabet] = None
  ): DLTS = {
    val nlts = NLTS.fromHOA(automatonString, fullAlphabet: Option[Alphabet])
    DLTS(
      nlts.name,
      NFAs.determinize(nlts.dfa, nlts.dfa.getInputAlphabet()).toFastDFA,
      nlts.alphabet
    )
  }

  /** @brief
    *   Build deterministic LTS from the DFA with accepting transitions as
    *   output by Spot's ltlf2dfa. A trace is accepted if the last transition is
    *   accepting.
    *
    * @param automatonString
    * @param acceptingLabel
    * @return
    */
  def fromHOAStringWithAcceptingTransitions(
      automatonString: String,
      fullAlphabet: Option[Alphabet] = None
  ): DLTS = {
    val nlts = NLTS.fromHOA(
      automatonString,
      fullAlphabet: Option[Alphabet],
      acceptingTransitions = true
    )
    DLTS(
      nlts.name,
      NFAs.determinize(nlts.dfa, nlts.dfa.getInputAlphabet()).toFastDFA,
  ...
  /** @brief
    *   Build deterministic LTS from possbly non-deterministic HOA Buchi
    *   automaton description: this will be interpreted
    *
    * @param automatonString
    * @param acceptingLabel
    * @return
  ...
  /** Create a regexp with the Brics library with the following syntactic sugar:
    * each event preceded by '@' is interpreted as an atomic letter. Example:
    * (~(.*@start1[^@end1]*@start1.*))&(~(.*@start2[^@end2]*@start2.*))
    *
    * @param name
    * @param regexp
    * @return
    ...
    def addIdentifier(): Boolean = {
      modifiedRegexp match {
        case identifierReg(name) =>
          ...
          modifiedRegexp =
            modifiedRegexp.replaceAllLiterally(s"@${name}", s"${currentChar}")
...
extension (lasso: Lasso) {
  ...
  /** @brief
    *   Determine whether given two lassos represent the same omega-word
    */
...
extension (dfa: FastDFA[String]) {
  ...
  /** Check if non-accepting states are traps (i.e. no accepting state is
    * reachable)
    *
    * @return
    */
  ...
  /** Copy the DFA by removing all non-accepting states and associated
    ...
    *
    * @return
    *   non-complete DFA equivalent to given DFA with all states accepting
    */
  def pruned = {
    val statesMap = HashMap((dfa.getInitialState(), FastDFAState(0, false)))
    val alphabet = dfa.getInputAlphabet()
...
object NLTS {
  def copy(nlts: NLTS): NLTS = {
    ...
    newNFA
      .getInitialStates()
      .foreach(s => newNFA.setInitial(statesMap(s), true))
    dfa
      .getStates()
      .foreach(
        { s =>
  ...
  /** Build a NLTS from the string description of a Buchi automaton in the HOA
    * format. The HOA format has atomic predicates (AP), and the transitions are
    * labeled by propositional formulas on AP. We build here NLTS where each
    * transition is labeled by a single symbol. We thus enumerate all singular
    * valuations, and keep only the transitions for these; moreover, for any
    * transition labeled by a valuation assigning false to all APs, we add
    * transitions labeled by fullAlphabet \\ APs.
    *
    * If [acceptingTransitions] is true, then we consider that a trace is
    * accepted if the last read transition is accepted. This is encoded in the
    * state space by adding one bit.
    *
    * @param automatonString
    *   description of the Buchi automaton in the HOA format
    * @param fullAlphabet
    *   the set of events on which to build the Buchi automaton; if None, then
    *   the alphabet of NLTS is the set of symbols that appear in the automaton
    *   description.
    * @param acceptingTransitions
    *   whether the described DFA has accepting transitions as output by Spot's
    *   ltlf2dfa.
    * @pre
    *   if fullAphabet is not None, then it contains all symbols that appear in
    *   the given automaton
    * @return
    */
  def fromHOA(
      automatonString: String,
      fullAlphabet: Option[Alphabet],
      acceptingTransitions: Boolean = false
  ): NLTS = {
    ...
    HOAFParser.parseHOA(
      new ByteArrayInputStream(automatonString.getBytes()),
      autFactory
    );
    ...
    val complementaryAlphabet = fullAlphabet match {
      ...
      case Some(symbols) =>
        if symbols.containsAll(alphabet) then {
          ...
          throw Exception(
            s"Cannot build NLTS from HOA: Not all atomic predicates of HOA are contained in the given alphabet: ${header
                .getAPs()} not contained in ${symbols}"
          )
        }
    }
    ...
    if acceptingTransitions then {
      require(properties.contains("trans-acc"))
    } else {
      require(properties.contains("state-acc"))
    }
    if (accCondition.getType() != BooleanExpression.Type.EXP_ATOM) then {
      throw Exception("Only Buchi acceptance is accepted")
    }
    if (accCondition.getAtom().getType() != AtomAcceptance.Type.TEMPORAL_INF)
    then {
      throw Exception("Only Buchi acceptance is accepted")
    }
    ...
    for
      a <- 0 until header.getAPs().size
      b <- 0 until header.getAPs().size
    do {
      if a != b then {
    ...
    if !acceptingTransitions then {
      ...
      header
        .getStartStates()
        .foreach(_.foreach({ i => nfa.setInitial(newStates(i), true) }))
    ...
    } else { // Transition-based acceptance
      // For each state index i, 2i is the version where the previous transition was accepting
      for i <- 1 to 2 * aut.getNumberOfStates() do {
        newStates.append(nfa.addState())
        if i >= aut.getNumberOfStates() then {
          nfa.setAccepting(newStates.last, true)
        }
      }
      header
        .getStartStates()
        .foreach(_.foreach({ i => nfa.setInitial(newStates(i), true) }))
      for i <- 0 until aut.getNumberOfStates() do {
        val s = newStates(i)
        val accepting_s = newStates(i + aut.getNumberOfStates())
        if (aut.getEdgesWithLabel(i) != null) then
          for edge <- aut.getEdgesWithLabel(i) do {
            assert(edge.getConjSuccessors().size == 1)
            val acc_edge = edge.getAccSignature() != null
            val succ = edge.getConjSuccessors().head
            val labels = singletonValuations(edge.getLabelExpr())
            ...
            for sigma <- labels do {
              if acc_edge then {
                nfa.addTransition(
                  s,
                  alphabet(sigma).toString,
                  newStates(succ + aut.getNumberOfStates())
                )
                nfa.addTransition(
                  accepting_s,
                  alphabet(sigma).toString,
            ...
            if (hasFalseValuation(edge.getLabelExpr())) then {
              for sigma <- complementaryAlphabet do {
                if acc_edge then {
                  nfa.addTransition(s, sigma, newStates(succ + aut.getNumberOfStates()))
                  nfa.addTransition(accepting_s, sigma, newStates(succ + aut.getNumberOfStates()))
                } else {
                  nfa.addTransition(s, sigma, newStates(succ))
                  nfa.addTransition(accepting_s, sigma, newStates(succ))
                }
              }
            }
          }
    ...
    if header.getName() != null then nlts.comments = header.getName()
    nlts
  }

  /** Build a NLTS which recognizes traces that start with an arbitrary prefix,
    * and then reads the given lasso. The NFA is defined as the union of two
    * parts. Either the lasso DFA, or another initial state with self-loops
    * which can go to the initial state of the lasso dfa.
    * @param lasso
    * @param alphabet
    * @return
    */
  def fromLassoAsSuffix(
      lasso: Lasso,
    ...
    val lassoInit =
      nlts.dfa
        .getInitialStates()
        .head // init state of the lasso. this remains initial
